...
Base Branch: {{base_branch}}`;
	...
	it("should ignore parent branch when issue has a parent (simplified branching)", async () => {
		...
		// Check that startStreaming was called with a prompt containing the REPOSITORY base branch (main)
		// NOT the parent branch
		expect(mockClaudeRunner.startStreaming).toHaveBeenCalled();
		const promptArg = mockClaudeRunner.startStreaming.mock.calls[0][0];
		expect(promptArg).toContain("Base Branch: main"); // Should match repo config
		expect(promptArg).not.toContain("Base Branch: parent-feature-branch");
	});

	it("should ignore parent branch even when parent has no branch name", async () => {
		...
		// Check that startStreaming was called with a prompt containing the repository base branch
		expect(mockClaudeRunner.startStreaming).toHaveBeenCalled();
		const promptArg = mockClaudeRunner.startStreaming.mock.calls[0][0];
		expect(promptArg).toContain("Base Branch: main");
	});

	it("should ignore deeply nested parent issues", async () => {
		...
		// Assert - should use repository base branch
		expect(vi.mocked(ClaudeRunner)).toHaveBeenCalled();
		expect(capturedClaudeRunnerConfig).toBeDefined();
		...
		const promptArg = mockClaudeRunner.startStreaming.mock.calls[0][0];
		expect(promptArg).toContain("Base Branch: main");
		expect(promptArg).not.toContain("parent-branch-456");
		expect(promptArg).not.toContain("grandparent-branch-789");
	});
});
